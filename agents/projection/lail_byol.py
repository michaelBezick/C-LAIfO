...
class LailByolAgent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
        ...
        if from_depth:
            ...
            self.byol = BYOL(self.encoder, (obs_shape[0]//3,) + tuple(obs_shape[-2:]), 
                             ...
                             projection_size=feature_dim).to(device)
    ...
    def act(self, obs, step, eval_mode):
        ...
        obs = self.encoder_head(self.byol.online_encoder(obs.unsqueeze(0), return_projection=False))
    ...
    def compute_reward(self, obs_a, next_a):
        ...
        with torch.no_grad():
            if self.from_dem:
                obs_a = self.encoder_head(self.byol.online_encoder(obs_a, return_projection=False))
            else:
                obs_a = self.encoder_head(self.byol.online_encoder(obs_a, return_projection=False))
                next_a = self.encoder_head(self.byol.online_encoder(next_a, return_projection=False))
    ...
    def update(self, replay_iter, replay_iter_expert, step):
        ...
        with torch.no_grad():
            obs_e = self.encoder_head(self.byol.online_encoder(obs_e, return_projection=False))
            next_obs_e = self.encoder_head(self.byol.online_encoder(next_obs_e, return_projection=False))
            obs_a = self.encoder_head(self.byol.online_encoder(obs_a, return_projection=False))
            next_obs_a = self.encoder_head(self.byol.online_encoder(next_obs_a, return_projection=False))
        ...
        # encode
        obs = self.encoder_head(self.byol.online_encoder(obs, return_projection=False))
        with torch.no_grad():
            next_obs = self.encoder_head(self.byol.online_encoder(next_obs, return_projection=False))
