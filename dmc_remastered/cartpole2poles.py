...
def get_model(visual_seed, vary=["camera", "light"]):
    default_model = _make_model(n_poles=2)
...
@register("cartpole2poles", "balance")
def balance(delta,
...
@register("cartpole2poles", "balance_sparse")
def balance_sparse(delta,
    time_limit=_DEFAULT_TIME_LIMIT, dynamics_seed=None, visual_seed=None, vary=DMCR_VARY
):
    model = get_model(visual_seed, vary)
    assets, _ = get_assets(visual_seed, delta, vary)
...
@register("cartpole2poles", "swingup")
def swingup(delta,
    time_limit=_DEFAULT_TIME_LIMIT, dynamics_seed=None, visual_seed=None, vary=DMCR_VARY
):
    model = get_model(visual_seed, vary)
...
@register("cartpole2poles", "swingup_sparse")
def swingup_sparse(
