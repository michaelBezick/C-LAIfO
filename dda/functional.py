...
from kornia.geometry.transform import shear, translate, rotate
...
def _gray(img: torch.Tensor) -> torch.Tensor:
    ...
    b,c,h,w = img.shape
    n_frames = c//3
    img = img.view([b, n_frames, 3, h, w])
    img = img[:, :, 0, ...]*0.299 + img[:, :, 1, ...]*0.587 + img[:, :, 0, ...]*0.110
    img = img[:, :, None, :, :]
    img = img * torch.ones([1, 1, 3, 1, 1]).to(device) #tiling
...
@tensor_function
def rotate_img(img: torch.Tensor, mag: torch.Tensor) -> torch.Tensor:
    return rotate(img, mag)
...
def hue(img: torch.Tensor, mag: torch.Tensor) -> torch.Tensor:
    ...
    b,c,hg,wd = img.shape
    n_frames = c//3
    img = img.view([b*n_frames, 3, hg, wd])
    h, s, v = _rgb_to_hsv(img).chunk(3, dim=1)

    if mag.shape[0] > 1:
        mag = mag.view(-1, 1, 1, 1).repeat(n_frames,1,1,1)
        
    _h = (h + mag) % 1
    img = _hsv_to_rgb(torch.cat([_h, s, v], dim=1))
    img = img.view([b, c, hg, wd])
